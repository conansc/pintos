...
syscall_handler (struct intr_frame *f)
{
  ...
  /* Check which syscall should be executed. Then, parse its arugments
 	...
 	switch(* (int*) addr) {
      ...
      parse_arguments(addr, argv, 2);
      validate_char_seq((void*) argv[1]);
      protect_char_sequence((char*) argv[1]);
      f->eax = sys_readdir(argv[0], (const char *) argv[1]);
      unprotect_char_sequence((char*) argv[1]);
      break;
    case SYS_ISDIR:                 /* Tests if a fd represents a directory. */
      parse_arguments(addr, argv, 1);
      f->eax = sys_isdir(argv[0]);
      break;
    case SYS_INUMBER:                 /* Returns the inode number for a fd. */
      parse_arguments(addr, argv, 1);
      f->eax = sys_inumber(argv[0]);
      break;
		default:
			break;
	}
...
int sys_open(char *file) {
  ...
  {
    return add_dir_to_process((struct dir *) f);
  }
  else
    return add_file_to_process(f);
}
...
int add_dir_to_process(struct dir* direc)
{
  struct thread_file *tf = malloc(sizeof(struct thread_file));
...
mapid_t sys_mmap(int fd, void * addr)
{
  ...
  if(t_file == NULL || file_length(t_file->file) == 0 || t_file->is_dir)
  {
...
/* Changes the cwd of the current thread given the directory name */
bool sys_chdir(const char* dir)
{
  return filesys_cd(dir);
}

/* Creates a new absolute or relative directory given the directory name */
bool sys_mkdir(const char* dir)
{
  return filesys_create(dir, 0, true);
}

/* Reads the directory name into the thread_file specified by fd */ 
bool sys_readdir(int fd, const char* name)
{
  ...
  if (!tf)
      ...
      return false;
  else if (!dir_readdir(tf->dir, (char *) name))
      return false;

  return true;
}

/* Checks if the thread_file of fd is a directory */
...
/* Returns the inumber of thread_file fd */
int sys_inumber(int fd)
{
  ...
  if (!tf->is_dir)
    ...
    return inode_get_inumber(dir_get_inode(tf->dir));
}
